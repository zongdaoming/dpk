...
class CKA:
    ...
    def compare_depth(self,
        ...
        for ((_, _), samples_masks_1, _), ((_, _), samples_masks_2, _) in tqdm(zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):
            x1 = samples_masks_1[0]
            x2 = samples_masks_2[0]
            # assert x1==x2, "Comparing input from two dataloaders"
    ...
    def compare_minibatches(self,
        ...
        # self.hsic_matrix = torch.zeros(N, M, 3)
        num_batches = min(len(dataloader1), len(dataloader1))
        batchsize_1 = dataloader1.batch_size
        batchsize_2 = dataloader2.batch_size
        assert batchsize_1 == batchsize_2
        batch_size = batchsize_1
        num_sub_batches = batch_size // 4
        # self.hsic_matrix = torch.zeros(num_batches, num_sub_batches, 3)
        self.hsic_matrix = torch.zeros(num_batches, 3)
        # for (x1, *_), (x2, *_) in tqdm(zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):
        idx = 0
        for ((_, _), samples_masks_1, _), ((_, _), samples_masks_2, _) in tqdm(zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):
            x1 = samples_masks_1[0]
            ...
            # _ = self.model2(x2.to(self.device))
            _ = self.model2(x1.to(self.device))
            # (Pdb) self.model1_features['module.layer4'].shape
            # torch.Size([32, 512, 7, 7])            
            # (Pdb) self.model2_features['module.layer4'].shape
            # torch.Size([32, 2048, 7, 7])
            # for i, (name1, feat1) in enumerate(self.model1_features.items()):
            # for j, (name2, feat2) in enumerate(self.model2_features.items()):
            # for j, index in enumerate(range(0, batch_size, 4)):
                # feat1 = self.model1_features['module.layer4'][index:index+4,:,:,:]
            # feat1 = self.model1_features['module.layer4']
            feat1 = self.model1_features['module.fc']
            X = feat1.flatten(1)
            K = X @ X.t()
            K.fill_diagonal_(0.0)
            self.hsic_matrix[idx, 0] += self._HSIC(K, K) / num_batches
                #self.hsic_matrix[idx].shape [16,3] 
                # self.hsic_matrix[idx, :, 0] += self._HSIC(K, K) / num_batches
                # tensor([[ 8684.3584,     0.0000,     0.0000],
                #         [ 8684.3584,     0.0000,     0.0000],
                #         [ 8684.3584,     0.0000,     0.0000],
                #         [ 8684.3584,     0.0000,     0.0000],
                #         [ 8684.3584,     0.0000,     0.0000],
                #         [ 8684.3584,     0.0000,     0.0000],
                ...
                #         [ 8684.3584,     0.0000,     0.0000],
                #         [ 8684.3584,     0.0000,     0.0000]])
                # dict_keys(['module.conv1', 'module.bn1', 'module.relu', 'module.maxpool', 'module.layer1.0.conv1', 'module.layer1.0.bn1', 'module.layer1.0.relu', 
                #  'module.layer1.0.conv2', 'module.layer1.0.bn2', 'module.layer1.0', 'module.layer1.1.conv1', 'module.layer1.1.bn1', 'module.layer1.1.relu', 
                # 'module.layer1.1.conv2',  'module.layer1.1.bn2', 'module.layer1.1', 'module.layer1', 'module.layer2.0.conv1', 
                #  'module.layer2.0.bn1', 'module.layer2.0.relu', 'module.layer2.0.conv2', 'module.layer2.0.bn2', 'module.layer2.0.downsample.0', 
                #  'module.layer2.0.downsample.1', 'module.layer2.0.downsample', 'module.layer2.0', 'module.layer2.1.conv1', 'module.layer2.1.bn1', 
                #  'module.layer2.1.relu', 'module.layer2.1.conv2', 'module.layer2.1.bn2', 'module.layer2.1', 'module.layer2', 'module.layer3.0.conv1', 
                #  'module.layer3.0.bn1', 'module.layer3.0.relu', 'module.layer3.0.conv2', 'module.layer3.0.bn2', 'module.layer3.0.downsample.0', 'module.layer3.0.downsample.1', 
                #  'module.layer3.0.downsample', 'module.layer3.0', 'module.layer3.1.conv1', 'module.layer3.1.bn1', 'module.layer3.1.relu', 'module.layer3.1.conv2', 
                # 'module.layer3.1.bn2', 
                #  'module.layer3.1', 'module.layer3', 'module.layer4.0.conv1', 'module.layer4.0.bn1', 'module.layer4.0.relu', 'module.layer4.0.conv2', 'module.layer4.0.bn2', 
                #  'module.layer4.0.downsample.0', 'module.layer4.0.downsample.1', 'module.layer4.0.downsample', 'module.layer4.0', 'module.layer4.1.conv1', 
                # 'module.layer4.1.bn1', 
                #  'module.layer4.1.relu', 'module.layer4.1.conv2', 'module.layer4.1.bn2', 'module.layer4.1', 'module.layer4', 'module.avgpool', 'module.fc', 'module', ''])                
                # feat2 = self.model2_features['module.layer4'][index:index+4,:,:,:]
            # feat2 = self.model2_features['module.layer4']
            feat2 = self.model2_features['module.fc']
            Y = feat2.flatten(1)
            L = Y @ Y.t()
            L.fill_diagonal_(0)
            assert K.shape == L.shape, f"Feature shape mistach! {K.shape}, {L.shape}"
                # self.hsic_matrix[idx, j, 1] += self._HSIC(K, L) / num_batches
                # self.hsic_matrix[idx, j, 2] += self._HSIC(L, L) / num_batches
            self.hsic_matrix[idx, 1] += self._HSIC(K, L) / num_batches
            self.hsic_matrix[idx, 2] += self._HSIC(L, L) / num_batches
            idx += 1
        self.hsic_matrix = self.hsic_matrix[:,  1] / (self.hsic_matrix[:, 0].sqrt() *
                                                        self.hsic_matrix[:, 2].sqrt())
    ...
    def plot_results(self,
        ...
        # only retain the top-100 sample hsic_matrix values
        # num_batches
        im = ax.imshow(self.hsic_matrix, origin='lower', cmap='Spectral_r')
        # im = ax.imshow(self.hsic_matrix.reshape(int(np.sqrt(self.hsic_matrix.shape[0])),int(np.sqrt(self.hsic_matrix.shape[0]))), origin='lower', cmap='Spectral_r', vim=-0.06, vmax=0.06)
        ax.set_xlabel(f"Layers {self.model2_info['Name']}", fontsize=15)
        # ax.set_xlabel(f"{self.model1_info['Name']}|{self.model2_info['Name']}", fontsize=15)
        ax.set_ylabel(f"Layers {self.model1_info['Name']}", fontsize=15)
        # ax.set_ylabel(f"Num_minibatches", fontsize=15)
